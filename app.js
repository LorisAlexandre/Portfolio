...
const suivOnClick = () => {
    ...
    const nextnextIndex = nextIndex + 1 <= articles.length - 1 ? nextIndex + 1 : 0;
    const preIndex = activeIndex - 1 >= 0 ? activeIndex - 1 : articles.length - 1;
    
    
    const currentArticle = document.querySelector(`[data-index="${activeIndex}"]`);
    const nextnextArticle = document.querySelector(`[data-index="${nextnextIndex}"]`);
...
const precOnClick = () => {
    ...
    const nextArticle = document.querySelector(`[data-index="${nextIndex}"]`);
    const preArticle = document.querySelector(`[data-index="${preIndex}"]`);
    const prepreArticle = document.querySelector(`[data-index="${prepreIndex}"]`);
